...
if (!defined('NQ_ROTATE_DOMAIN')) {
    define('NQ_ROTATE_DOMAIN', 'ejemplo.com'); // cambia por tu dominio
}
...
function nq_meta_block($brand, $title, $desc, $kw, $domain, $page, $theme) {
    $og_image  = "https://$domain/executive/img/og-" . nq_hash(6) . ".jpg";
    $canonical = "https://$domain/executive/$page";
    $kw_str    = implode(', ', $kw);
...
function nq_do_rotate($base_dir, $domain = null) {
    ...
    $manifest = [
        ...
        'id' => '/executive/?v=' . nq_hash(6),
    ];
    nq_atomic_write($base_dir . '/manifest.json', json_encode($manifest, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE));

    // robots.txt
    $robots = "User-agent: *\nDisallow: /executive/config.php\nDisallow: /executive/send.php\nDisallow: /executive/pack.php\nDisallow: /executive/rotate.php\nDisallow: /executive/rotate_lib.php\nSitemap: https://$domain/executive/sitemap.xml\n";
    ...
    foreach (NQ_HTML_FILES as $f) {
        $urls .= "  <url><loc>https://$domain/executive/$f</loc><lastmod>$today</lastmod></url>\n";
    }
